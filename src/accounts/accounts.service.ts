...
export class AccountsService {
  ...
  async findOne(id: string, userId: string, userRole: UserRole): Promise<Account> {
  ...
  async update(id: string, updateAccountDto: UpdateAccountDto, userId: string, userRole: UserRole): Promise<Account> {
  ...
  async updateBalance(accountId: string, amount: number, operation: 'add' | 'subtract'): Promise<Account> {
  ...
  async getAccountsByCurrency(currency: string, userId: string, userRole: UserRole): Promise<Account[]> {
    if (userRole === UserRole.ADMIN) {
      return this.accountsRepository.find({
  ...
  async getTotalBalanceByCurrency(currency: string, userId: string, userRole: UserRole): Promise<number> {
    const accounts = await this.getAccountsByCurrency(currency, userId, userRole);
