...
export class TransactionsService {
  ...
  async create(createTransactionDto: CreateTransactionDto, userId: string, userRole: UserRole): Promise<Transaction> {
    ...
    try {
      ...
      if (userRole !== UserRole.ADMIN && account.userId !== userId) {
        throw new ForbiddenException('You can only create transactions for your own accounts');
      }
  ...
  async update(id: string, updateTransactionDto: UpdateTransactionDto, userId: string, userRole: UserRole): Promise<Transaction> {
  ...
  async getTransactionsByType(type: TransactionType, userId: string, userRole: UserRole): Promise<Transaction[]> {
    if (userRole === UserRole.ADMIN) {
      return this.transactionsRepository.find({
        where: { type },
        relations: ['sourceUser', 'targetUser', 'account'],
